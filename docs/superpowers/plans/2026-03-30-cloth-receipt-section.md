# Cloth Receipt Section Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add an interactive 3D cloth physics receipt section at the top of the portfolio (before IntroSection) using WebGL + Verlet integration — user can grab and drag the receipt like a piece of cloth, and still scroll freely.

**Architecture:** A new `ClothReceiptSection` React component wraps a full-screen `<canvas>` element. All WebGL setup, cloth physics, and interaction logic lives inside a single `useEffect` that initializes on mount and cleans up on unmount. The receipt texture is generated programmatically on an offscreen Canvas 2D — no external image assets needed.

**Tech Stack:** React, TypeScript, WebGL (native), Canvas 2D (texture generation), Verlet cloth physics (Position-Based Dynamics), Next.js dynamic import (ssr: false)
...
| File | Action | Purpose |
|---|---|---|
| `src/components/sections/ClothReceiptSection.tsx` | Create | Full component — WebGL canvas + cloth physics + interaction |
| `src/app/page.tsx` | Modify | Import and render ClothReceiptSection before IntroSection |

---

### Task 1: Create ClothReceiptSection component skeleton

**Files:**
- Create: `src/components/sections/ClothReceiptSection.tsx`

- [ ] **Step 1: Create the file with canvas mount skeleton**
...
```bash
ls src/components/sections/ClothReceiptSection.tsx
```
...
```bash
git add src/components/sections/ClothReceiptSection.tsx
git commit -m "feat: add ClothReceiptSection skeleton"
```

---

### Task 2: Add receipt texture generation (Canvas 2D)

**Files:**
- Modify: `src/components/sections/ClothReceiptSection.tsx`

- [ ] **Step 1: Add texture generation function before the component**

Insert this function before `export default function ClothReceiptSection()`:
...
```bash
git add src/components/sections/ClothReceiptSection.tsx
git commit -m "feat: add receipt texture generation"
```

---

### Task 3: Add WebGL shaders and program setup

**Files:**
- Modify: `src/components/sections/ClothReceiptSection.tsx`

- [ ] **Step 1: Add shader sources and helpers after the imports, before `buildReceiptTexture`**
...
```bash
git add src/components/sections/ClothReceiptSection.tsx
git commit -m "feat: add WebGL shaders and matrix helpers"
```

---

### Task 4: Add cloth physics data structures

**Files:**
- Modify: `src/components/sections/ClothReceiptSection.tsx`

- [ ] **Step 1: Add cloth particle and constraint types after the helpers**
...
```bash
git add src/components/sections/ClothReceiptSection.tsx
git commit -m "feat: add cloth physics data structures"
```

---

### Task 5: Implement full WebGL + physics render loop in useEffect

**Files:**
- Modify: `src/components/sections/ClothReceiptSection.tsx`

- [ ] **Step 1: Replace the useEffect body with full implementation**
...
```bash
git add src/components/sections/ClothReceiptSection.tsx
git commit -m "feat: implement full WebGL cloth physics render loop"
```

---

### Task 6: Add "Scroll to explore" hint overlay

**Files:**
- Modify: `src/components/sections/ClothReceiptSection.tsx`

- [ ] **Step 1: Update the return JSX to add hint text**
...
```bash
git add src/components/sections/ClothReceiptSection.tsx
git commit -m "feat: add hint overlay to cloth receipt section"
```

---

### Task 7: Mount section in page.tsx

**Files:**
- Modify: `src/app/page.tsx`

- [ ] **Step 1: Add dynamic import for ClothReceiptSection**

In `src/app/page.tsx`, add after the existing dynamic imports (after line `const ScannerEffect = ...`):

```tsx
const ClothReceiptSection = dynamic(() => import('@/components/sections/ClothReceiptSection'), { ssr: false });
```

- [ ] **Step 2: Render ClothReceiptSection before IntroSection**

In the `<main>` block, insert before `<IntroSection />`:
...
npm run dev
```
Expected: No TypeScript or build errors. Open `http://localhost:3000` — cloth receipt section should appear as the first section, receipt hangs and sways, grabbable by mouse.

- [ ] **Step 4: Commit**

```bash
git add src/app/page.tsx
git commit -m "feat: mount ClothReceiptSection before IntroSection"
```
...
If the grep above returns nothing, add to `.gitignore`:
